...
async function rcExpl(key, gp, q, cq) {
  ...
  let pr = tp?.template || `Year 6 teacher. RC explanation. State correct answer. Explain each option. Highlight evidence with <mark class="bg-yellow-200 px-1 rounded">...</mark>. Return JSON: {"advice":"...","highlightedContent":"..."} or {"advice":"...","passages":[{"passageId":"...","highlightedContent":"..."}]}.
Question: {{QUESTION}}
Passage(s): {{PASSAGES}}
Options: {{OPTIONS}}
      ...
      for (let qi = 0; qi < qs.length; qi++) {
        ...
        try {
          if (q.type === 'reading_comprehension') {
            ...
            if (wE && !q.ai_data?.explanation) { const r = { advice: await gem(key, `Year 6 teacher. Explain MC. State correct answer, explain each option. HTML.\nQ: ${s(q.question)}\nOpts: ${(q.options||[]).join(', ')}\nCorrect: ${q.correctAnswer}`) }; qs[qi] = { ...qs[qi], ai_data: { ...qs[qi].ai_data, explanation: r } }; ch = true; st.explanations++; }
            if (wT && !q.ai_data?.helper_tip) { const r = { advice: await gem(key, `Year 6 teacher. Hint for MC. Do NOT reveal answer. HTML.\nQ: ${s(q.question)}\nOpts: ${(q.options||[]).join(', ')}`) }; qs[qi] = { ...qs[qi], ai_data: { ...qs[qi].ai_data, helper_tip: r } }; ch = true; st.tips++; }
          } else if (q.type === 'drag_drop_single' || q.type === 'drag_drop_dual') {
            const px = pc(q);
            for (let zi = 0; zi < (q.dropZones || []).length; zi++) {
              const z = q.dropZones[zi];
              if (wE && !z.ai_data?.explanation) { const r = { advice: await gem(key, `Year 6 teacher. Explain drag-drop "${z.label}". Correct: "${z.correctAnswer}". HTML.${px ? '\n'+px : ''}`) }; qs[qi].dropZones[zi] = { ...qs[qi].dropZones[zi], ai_data: { ...qs[qi].dropZones[zi].ai_data, explanation: r } }; ch = true; st.explanations++; }
              if (wT && !z.ai_data?.helper_tip) { const r = { advice: await gem(key, `Year 6 teacher. Hint for drag-drop "${z.label}". Do NOT reveal answer. HTML.${px ? '\n'+px : ''}\nOpts: ${(q.options||[]).join(', ')}`) }; qs[qi].dropZones[zi] = { ...qs[qi].dropZones[zi], ai_data: { ...qs[qi].dropZones[zi].ai_data, helper_tip: r } }; ch = true; st.tips++; }
            }
            ...
            for (let bi = 0; bi < (q.blanks || []).length; bi++) {
              ...
              if (wT && !b.ai_data?.helper_tip) { let pt = q.textWithBlanks ? s(q.textWithBlanks) : q.passage ? s(q.passage) : ''; const r = { advice: await gem(key, `Year 6 teacher. Hint for blank ${bi+1}. Do NOT reveal answer. HTML.\nOpts: ${b.options.join(', ')}\nPassage:\n${pt||'No passage'}`) }; qs[qi].blanks[bi] = { ...qs[qi].blanks[bi], ai_data: { ...qs[qi].blanks[bi].ai_data, helper_tip: r } }; ch = true; st.tips++; }
            }
          } else if (q.type === 'matching_list_dual') {
            const px = pc(q);
            for (let mi = 0; mi < (q.matchingQuestions || []).length; mi++) {
              const mq = q.matchingQuestions[mi];
              if (wE && !mq.ai_data?.explanation) { const r = { advice: await gem(key, `Year 6 teacher. Explain matching "${mq.question}". Correct: "${mq.correctAnswer}". HTML.${px ? '\n'+px : ''}`) }; qs[qi].matchingQuestions[mi] = { ...qs[qi].matchingQuestions[mi], ai_data: { ...qs[qi].matchingQuestions[mi].ai_data, explanation: r } }; ch = true; st.explanations++; }
              if (wT && !mq.ai_data?.helper_tip) { const r = { advice: await gem(key, `Year 6 teacher. Hint for matching "${mq.question}". Do NOT reveal answer. HTML.${px ? '\n'+px : ''}`) }; qs[qi].matchingQuestions[mi] = { ...qs[qi].matchingQuestions[mi], ai_data: { ...qs[qi].matchingQuestions[mi].ai_data, helper_tip: r } }; ch = true; st.tips++; }
            }
          }
