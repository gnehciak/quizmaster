...
import { Plus, Trash2, ChevronLeft, Pencil, FileQuestion, GripVertical, Search, Save, X } from 'lucide-react';
...
export default function ManageCategories() {
  ...
  const updateOrderMutation = useMutation({
    mutationFn: (updates) => Promise.all(updates.map(u => base44.entities.QuizCategory.update(u.id, { order: u.order }))),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['quizCategories'] });
    }
  });
  ...
  }, [categories, searchTerm, sortBy, quizCounts]);

  const handleDragEnd = (result) => {
    if (!result.destination) return;
    if (sortBy !== 'custom') return; // Only allow reorder when in custom sort mode

    const items = Array.from(sortedCategories);
    const [reorderedItem] = items.splice(result.source.index, 1);
    items.splice(result.destination.index, 0, reorderedItem);

    // Optimistic update could happen here, but we'll wait for server for simplicity or implement local state if needed
    // For now, let's trigger the update
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Organize your quiz categories</p>
              </div>
            </div>
